...
export default function SiteFooter() {
  return (
    <footer className="bg-ink text-on-ink-body border-t border-line-on-ink">
      <div className="mx-auto max-w-[1280px] px-[clamp(20px,3vw,40px)] pt-[clamp(48px,5vw,72px)] pb-9 grid grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-[clamp(24px,3vw,40px)]">
        <div className="min-w-[200px]">
          <div className="font-display font-extrabold type-body text-white tracking-[-.02em] mb-3.5">
            hyprr <span className="font-normal text-on-ink-mute">brands</span>
          </div>
        ...
        {COLUMNS.map((col) => (
          <div key={col.title}>
            <div className="type-label text-on-ink-mute uppercase mb-4">
              {col.title}
            </div>
            <div className="grid gap-2.5 type-meta">
              {col.links.map(([href, label]) => (
                <a
                  key={href}
                  href={href}
                  className="text-on-ink-body hover:text-white"
                >
                  {label}
                </a>
              ))}
      ...
      <div className="mx-auto max-w-[1280px] px-[clamp(20px,3vw,40px)] pt-6 pb-11 type-meta flex justify-between gap-[18px] flex-wrap border-t border-line-on-ink">
        <span>© 2026 Hyprr Retail LLC</span>
        <div className="flex gap-5 flex-wrap">
          <a href="/privacy" className="text-on-ink-body hover:text-white">
            Privacy policy
          </a>
          <a href="/terms" className="text-on-ink-body hover:text-white">
            Terms of service
          </a>
          <a href="/disclosure" className="text-on-ink-body hover:text-white">
            Disclosure
          </a>
          <a href="/accessibility" className="text-on-ink-body hover:text-white">
            Accessibility
          </a>
        </div>
      </div>
